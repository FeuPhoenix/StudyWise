...
class audiocutter:
    def __init__(self,INPUT_FILE):
        self.TEMP_FOLDER = "mainServerTest/assets/output_files/Temp_Files/TEMP" 

        self.start_time = time.time()
        self.runaudiocutter(INPUT_FILE)
    
    def getMaxVolume(self,s): # get max volume of a given audio file (we also find the min volume because audio signals signals can have negative values)
        maxv = float(np.max(s))
        minv = float(np.min(s)) 
        return max(maxv,-minv) 
    
    def copyFrame(self,inputFrame,outputFrame): # copy frame inputFrame and save it as outputFrame
    ...
    def createPath(self,s): # Create directory structure for a given path
        try:
            if os.path.exists(s):
    ...
    def deletePath(self,s): # Dangerous! Watch out!
        try:
    ...
    def getFileNameFromPath(self,input_string):
    ...
    def runaudiocutter(self,INPUT_FILE):
        #Parameters
        
        FILE_NAME= self.getFileNameFromPathWithOutExtension(INPUT_FILE)
        ...
        command = "ffmpeg -i "+INPUT_FILE+" -qscale:v "+str(FRAME_QUALITY)+" "+self.TEMP_FOLDER+"/frame%06d.jpg -hide_banner" # extract frames from input video
        subprocess.call(command, shell=True)

        command = "ffmpeg -i "+INPUT_FILE+" -ab 160k -ac 2 -ar "+str(SAMPLE_RATE)+" -vn "+self.TEMP_FOLDER+"/audio.wav" # extract audio from input video

        subprocess.call(command, shell=True) 

        command = "ffmpeg -i "+self.TEMP_FOLDER+"/input.mp4 2>&1" # get video info
        f = open(self.TEMP_FOLDER+"/params.txt", "w") # write video info to file
        subprocess.call(command, shell=True, stdout=f)



        sampleRate, audioData = wavfile.read(self.TEMP_FOLDER+"/audio.wav") # read audio file
        audioSampleCount = audioData.shape[0] # number of audio samples
        maxAudioVolume =self.getMaxVolume(audioData) # max volume of audio file

        f = open(self.TEMP_FOLDER+"/params.txt", 'r+') 
        pre_params = f.read()
        f.close() 
        ...
        for i in range(audioFrameCount): # check if audio frame has loud audio
            ...
            maxchunksVolume = float(self.getMaxVolume(audiochunks))/maxAudioVolume # max volume of audio data for a given frame
        ...
        wavfile.write(self.TEMP_FOLDER+"/audioNew.wav",SAMPLE_RATE,outputAudioData) # write audio data to file
            
        # OUTPUT VIDEO CREATION
        command = "ffmpeg -framerate "+str(frameRate)+" -i "+self.TEMP_FOLDER+"/newFrame%06d.jpg -i "+self.TEMP_FOLDER+"/audioNew.wav -strict -2 "+OUTPUT_FILE # combine frames and audio to create output video
        subprocess.call(command, shell=True)
