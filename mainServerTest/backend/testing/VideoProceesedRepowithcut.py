...
class VideoProcessed_Repo_withcut:
    ...
    def addProcessedMaterialToFirestore(self):
        ...
        file_path_location,generated_text_file_path_Location\
            ...
            ,generated_chapters_file_path_Location,generated_audio_file_path_Location=VideoProcessed_Repo_withcut.upload_material_to_storage(self.user_ID,self.file_name , self.file_path,self.generated_text_file_path,self.generated_summary_file_path,self.generated_chapters_file_path,self.generated_audio_file_path)
    ...
    def upload_material_to_storage(user_id, material_name, material_file_path, text_file_path, summary_file_path, chapters_file_path, audio_file_path):
        ...
        VideoProcessed_Repo_withcut.upload_Video_file_to_storage(material_blob, material_file_path)
        ...
        VideoProcessed_Repo_withcut.upload_audio_file_to_storage(audio_blob, audio_file_path)
    ...
    def addProcessedMaterialToFirestore_youtubeLink(self):
        ...
        generated_text_file_path_Location\
            ,generated_summary_file_path_Location\
            ,generated_chapters_file_path_Location,generated_audio_file_path_Location=VideoProcessed_Repo_withcut.upload_material_youtube_to_storage(self.user_ID,self.file_name  ,self.generated_text_file_path,self.generated_summary_file_path,self.generated_chapters_file_path,self.generated_audio_file_path)
    ...
    def upload_material_youtube_to_storage(user_id, material_name,  text_file_path, summary_file_path, chapters_file_path, audio_file_path):
        ...
        audio_blob_path = folder_path + "audio.mp3"
        audio_blob = storage_instance.blob(audio_blob_path)
        VideoProcessed_Repo_withcut.upload_audio_YT_file_to_storage(audio_blob, audio_file_path)
    ...
    def Video_Processing(self):
        self.file_name = VideoProcessed_Repo_withcut.getFileNameFromPathWithOutExtension(self.material)

        try:
            if self.is_mp4_file(self.material):
                self.file_type = "mp4"
                self.file_path = self.material
                self.meta_data = VideoProcessed_Repo_withcut.get_video_metadata(self.material)
                if VideoProcessed_Repo_withcut.check_value_exists_in_VideoMaterial(self.user_ID,self.meta_data):
                    ...
                    self.file_name = VideoProcessed_Repo_withcut.getFileNameFromPathWithOutExtension(self.file_path)
                    ...
                    for chapter in chapters_data:
                        start_hms = VideoProcessed_Repo_withcut.milliseconds_to_hms(chapter['start'])
                        end_hms = VideoProcessed_Repo_withcut.milliseconds_to_hms(chapter['end'])
                        concise_title = VideoProcessed_Repo_withcut.generate_concise_title(chapter['headline'])
                ...
                if VideoProcessed_Repo_withcut.check_value_exists_in_VideoMaterial(self.user_ID, self.meta_data):
                    self.file_name = VideoProcessed_Repo_withcut.download_audio_with_title(self.material)
...
def main():
    ...
    a = VideoProcessed_Repo_withcut("C:/Users/Abdelrahman/Downloads/Coding_Basics_-_Variables_-_Programming_for_Beginners.mp4","jo45D5quBvRHU9q0iDLdYbcfdNI2",True)
    a.Video_Processing()
    print(a.file_name)
